...
import { getCargosList, deleteCargos } from "../../../redux/actions/cargoActions";
...
import { CARGO_DELETE_RESET } from "../../../redux/constants/cargoConstants";
...
function CargoListScreen({ history }) {
  ...
  const cargoList = useSelector((state) => state.cargosList);
  const { loading, error, cargos } = cargoList;
  // Delete Provinces Selector
  const deleteCargoSelector = useSelector((state) => state.cargosDelete);
  const { success: successDelete, error: errorDelete } = deleteCargoSelector;
    ...
    if (successDelete) {
      dispatch(
        setSnackbar(
          true,
          "success",
          "Cargo(s) eliminado(s) satisfactoriamente"
        )
      );
      dispatch({ type: CARGO_DELETE_RESET });
    }
  ...
  listOptions.customToolbarSelect = ({ data }) => {
    return (
      <React.Fragment>
        <Tooltip title="Eliminar Cargo(s) Seleccionados" className="mr-2">
          <IconButton
            onClick={() => {
              let items = [];
              data.forEach((element) => {
                items.push(cargos[element.dataIndex]);
              });
  ...
  const deleteComfirmedItems = (items) => {
    dispatch(deleteCargos(items));
  };

  // Add Buttom to insert to the table
  listOptions.customToolbar = () => {
    return <AddButtomListHeader addLink="/cargos/add" title="Insertar Cargo" />;
  };
      ...
      {loading ? (
        ...
        <div>
          <MuiThemeProvider theme={getMuiTheme()}>
            <MUIDataTable
              title={`Listado de Cargos(${cargos?.length})`}
              data={cargos}
              columns={columns}
              options={listOptions}
            />
