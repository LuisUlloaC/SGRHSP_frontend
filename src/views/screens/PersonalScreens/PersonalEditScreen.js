...
import { validationSchema } from "./options/personalValidationSchema";
...
function PersonalEditScreen({history, match }) {
  const dispatch = useDispatch();
  const personalId = match.params.id;
  ...
  const { loading: loadingDetails, error: errorDetails} = personalDetails;

  const editPersonalSelector = useSelector((state) => state.personalEdit);
  const { loading, error, success } = editPersonalSelector;
  ...
  //Selector
  const {plaza} = useSelector((store) => store.plazasList);
  let loadedPlaza = plaza ? plaza : [];
  ...
  useEffect(() => {
    if (!userInfo) {
      ...
      if (personalId) {
        dispatch(getPersonalDetails(personalId, false));
      }
      if (success) {
        dispatch(
          setSnackbar(true, "success", "Personal editado satisfactoriamente")
        );
        dispatch({ type: OBJECT_EDIT_RESET });
        history.push("/personal");
      }
    }
  }, [userInfo, history, success, dispatch, personalId]);
  ...
  useEffect(function loadUtils(){
    ...
    return () => {
      dispatch({type: OBJECT_LIST_RESET, PLAZA_LIST_RESET, MOTIVOBAJA_LIST_RESET})
    }
  },[dispatch])
  ...
  const formik = useFormik({
    ...
    initialValues: {nombre:"",apellidos:"",sexo:"",ci:"",direccionParticular:"",fechaAlta:"",fechaBaja:"",
      fuenteProcedencia:fuentesProcedencia[0]?.id, motivoBaja:loadedMotivos[0]?.id, plaza:plaza[0]?.id},
    validationSchema: validationSchema,
    validateOnChange: true,
    validateOnBlur: true,
    onSubmit: (values) => {
      dispatch(editPersonal(personalId, values));
    },
  });
        ...
        <CRow>
          <CCol xs="12">
            ...
            <CCard className="shadow">
              <CCardHeader>
                Formulario para Editar <strong>Personal</strong>
                <CButton
                  variant="outline"
                  color="light"
...
export default PersonalEditScreen;
